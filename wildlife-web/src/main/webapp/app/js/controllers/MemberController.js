...
sydneyWildlifeApp
    .controller(
        'MemberController',
        function MemberController($scope, $window, $timeout, MemberService, NavService,
            AlertService, $routeParams, NAV_PATHS, USER_ROLES, ALERT_CODES, ngTableParams) {
          $scope.memberList = {};
          ...
          $scope.saveMember = function(member, form) {
            if (form.$valid) {
              ...
              aPromise.then(function(object) {
                $scope.stopLoading();
                member.id = object.id;
                AlertService.show(ALERT_CODES.success, "Successfully saved member #" + member.id
                    + "(" + member.firstName + " " + member.lastName + ").");
                $scope.goBack();
              }, function errorCallback(error) {
                $scope.stopLoading();
                $scope.errors.push("We cannot save the member #" + member.id + " ("
                    + member.firstName + " " + member.lastName
            ...
            MemberService
                ...
                .then(
                    ...
                    function(error) {
                      $scope.stopLoading();
                      $scope.errors
                          .push("We cannot retrieve the list of members at the moment, please try again later.");
          ...
          $scope.memberDetail = function() {
            if ($routeParams != undefined && $routeParams.memberId != undefined) {
              ...
              MemberService.memberDetail($routeParams.memberId).get().then(
                  ...
                  function(error) {
                    $scope.stopLoading();
                    $scope.errors.push("We cannot retrieve the member #" + $routeParams.memberId
                        + " at the moment, please try again later.");
                    AlertService.show(ALERT_CODES.error, "Error retrieving member with Id "
          ...
          $scope.deleteMember = function(member) {
            if (member != undefined && member.id != undefined) {
              ...
              $timeout(function() {
                var deleteConfirmed = $window
                    .confirm("Are you sure you want to permanently delete member "
                        + member.firstName + " " + member.lastName + "?");
                if (deleteConfirmed) {
                  $scope.startLoading();
                  MemberService.deleteMember(member.id).then(
                      function(object) {
                        $scope.stopLoading();
                        $scope.member = {};
                        AlertService.show(ALERT_CODES.info, "Successfully deleted member with Id "
                            + member.id + ".");
                        $scope.goBack();
                      },
                      function(error) {
                        $scope.stopLoading();
                        $scope.errors.push("We cannot delete the member #" + member.id + " ("
                            + member.firstName + " " + member.lastName
                            + ") at the moment, please try again later.");
                        AlertService.show(ALERT_CODES.error, "Error deleting member with Id "
                            + member.id + ".");
                      });
                }
              });
          ...
          /**
           * Goes back to the Members List page id admin role or to the home
           * page otherwise
           */
          ...
          /**
           * Returns true if the member object is a new one, i.e. it hasn't been
           * saved to the DB yet.
           */
          $scope.isNew = function() {
            return !$scope.isLoading()
                && ($scope.member == undefined || $scope.member.id == undefined);
          };
          ...
          /**
           * Returns a string value representing a form mode: "create", "read"
           * or "update". If any error occurred, the result will be "error".
           */
